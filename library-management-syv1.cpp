...
{
public:
    Borrow(string Bid = "None",string Uid = "None",string BorrowDate = "None" ,string ReturnDate = "None"):nBookId(Bid),nUserId(Uid),strBorrowDate(BorrowDate),strReturnDate(ReturnDate){
    }
	void ShowBorrow(){
		cout << "The Book ID: " << nBookId << endl;
		cout << "The User ID: " << nUserId << endl;
		cout << "The BorrowDate: " << strBorrowDate << endl;
		cout << "The ReturnDate: " << strReturnDate << endl;
	}
	void SetBorrowDate(){
		cout << "Enter Book BorrowDate: " << endl;
		getline(cin,this->strBorrowDate);
        cout << "Save Compeletely!" << endl;
    }
	void SetReturnDate(){
		cout << "Enter Book ReturnDate: " << endl;
		getline(cin,this->strReturnDate);
        cout << "Save Compeletely!" << endl;
    }
public:
	string nBookId;					//图书ID
	string nUserId;					//用户ID
	string strBorrowDate;			//借阅时间
	string strReturnDate;			//实际归还时间
};
/// @brief //////////////////////////////////////////////
/// @return 
Borrow Borrows[100];///定义一个全局变量borrows来存储borrow信息
int B = 0;
...
int main() {   
    ...
    while (true) {
        ...
        if (command == "add") {
            ...
            cout << "Your Borrow Num: " << B<<endl;
            string id;
            int quantity;
            cout << "Enter book ID to lend: ";
            cin >> id;
            Borrows[B].nBookId = id;
            cout << "Enter quantity: ";
            cin >> quantity;
            lib.Lend(id, quantity);
            cout << "Enter Today's Date: " << endl;
            cin>> Borrows[B].strBorrowDate;
            cout << "Enter User ID: ";
            cin >> Borrows[B].nUserId;

            B++;
            //////////////////////////////////////////////
        } else if (command == "return") {
            int BO;
            cout << "Enter Your Borrow Num: " << endl;
            cin >> BO;
            string id;
            int quantity;
